...
int leapRun(const char* pkg_name, int silent) {
    ...
    if (!silent) {("Extracting %s to .leapbuffer...", pkg_name);}
    if (system(com) != 0) {
    ...
    if (!silent) {("Checking existence of metadata.json...");}
    ...
    if (!silent) {("Running Python script to parse metadata.json...");}
    fp = popen(com, "r");
    if (fp == NULL) {
    ...
    if (!silent) {("Output from Python script: %s", output);}
    ...
    if (!silent) {("Successfully processed metadata.json. Proceeding...");}
    char runcom[512];
